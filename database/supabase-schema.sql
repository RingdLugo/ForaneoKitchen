...
create table if not exists public.chatbot_memorias (
  id integer generated by default as identity,
  usuario_id uuid not null unique,
  memoria jsonb not null default '{}'::jsonb,
  updated_at timestamptz default now(),
  constraint chatbot_memorias_pkey primary key (id),
  constraint chatbot_memorias_usuario_id_fkey foreign key (usuario_id) references public.usuarios(id)
);
...
create table if not exists public.chatbot_conversaciones (
  ...
  constraint chatbot_conversaciones_usuario_id_fkey foreign key (usuario_id) references public.usuarios(id)
);

create index if not exists chatbot_conversaciones_usuario_fecha_idx
  on public.chatbot_conversaciones (usuario_id, created_at desc);
